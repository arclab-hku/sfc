...
struct Plane {
  Plane() {}
  Plane(const Vecf<Dim>& p, const Vecf<Dim>& n) : p_(p), n_(n) {}
...
///Plane2D: first is the point on the hyperplane, second is the normal
typedef Plane<2> Plane2D;
///Plane3D: first is the point on the hyperplane, second is the normal
...
struct Polyhedron {
  ///Null constructor
  Polyhedron() {}
  ///Construct from Plane array
  Polyhedron(const vec_E<Plane<Dim>>& vs) : vs_(vs) {}


  ///Append Plane
  void add(const Plane<Dim>& v) {
    vs_.push_back(v);
  }
  ...
  vec_E<Plane<Dim>> hyperplanes() const {
    return vs_;
  }

  /// Plane array
  vec_E<Plane<Dim>> vs_; // normal must go outside
...
struct LinearConstraint {
   ...
   */
	LinearConstraint(const Vecf<Dim> p0, const vec_E<Plane<Dim>>& vs) {
